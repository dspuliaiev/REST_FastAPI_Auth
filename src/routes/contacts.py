...
async def read_contacts(limit: int = Query(10, ge=10, le=500), offset: int = Query(0, ge=0),
    ...
    contacts = await repositories_contacts.read_contacts(limit=limit, offset=offset, db=db, user=user)
    return contacts
...
async def read_contact(contact_id: int, db: AsyncSession = Depends(get_db),
                      user: User = Depends(auth_service.get_current_user)):
    contact = await repositories_contacts.read_contact(contact_id=contact_id, db=db, user=user)
    if contact is None:
...
async def update_contact(body: ContactUpdateSchema, contact_id: int = Path(ge=1), db: AsyncSession = Depends(get_db),
                        user: User = Depends(auth_service.get_current_user)):
    contact = await repositories_contacts.update_contact(contact_id, body=body, db=db, user=user)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="NOT FOUND")
    return contact
